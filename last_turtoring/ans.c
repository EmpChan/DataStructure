...
{
    struct Word_Counter* new_cnt = (struct Word_Counter*)malloc(sizeof(struct Word_Counter));
...
void insert_to_list(struct Word_Counter** head,char* item)//ordered linked list
{
    ...
    else
    {
        ...
        while(tmp->next!=NULL)
        {
            if(strcmp(tmp->next->item , item) < 0)
            {
                tmp=tmp->next;
            }    
            else if(strcmp(tmp->next -> item , item) == 0)
            {
                tmp->next->cnt++;
                return;
            }
            ...
            {
                struct Word_Counter* cup = tmp->next;
